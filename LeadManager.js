...
https
  .createServer(options, function (res, req) {
    app.use(
      urlencoded({
        extended: true
      })
    ...
    app.set("view engine", "ejs"); // Check if all environment variables are set
      ...
      if (mode && token) {
        if (token === config.verifyToken) {
          ...
          if (mode == "private-reply") {
            ...
            res.write(
              `<p>&#9989; Set Page Feed Webhook for Private Replies.</p>`
            );
          }
          res.status(200).end();
        } else {
